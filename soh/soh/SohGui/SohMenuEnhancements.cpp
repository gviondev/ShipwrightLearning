...
void SohMenu::AddMenuEnhancements() {
    ...
    AddWidget(path, "Damage Dealt (%)", WIDGET_CVAR_INPUT)
        ...
        .Options(InputOptions()
                     ...
                     .Tooltip("Modifies damage dealt by Link and his projectiles.\n"
                              "Accepts decimal percentages for fine tuning. 100% is vanilla."))
        .Callback([clampDamagePercent](WidgetInfo& info) {
            clampDamagePercent(CVAR_ENHANCEMENT("PlayerDamageDealtPercent"));
        });
    ...
    AddWidget(path, "Hyper Bosses", WIDGET_CVAR_CHECKBOX)
        ...
        .Options(CheckboxOptions().Tooltip(
            "All Major Bosses move and act faster based on the Hyper Speed Increase sliders."));
    AddWidget(path, "Hyper Enemies", WIDGET_CVAR_CHECKBOX)
        .CVar(CVAR_ENHANCEMENT("HyperEnemies"))
        .Options(CheckboxOptions().Tooltip(
            "All Regular Enemies and Mini-Bosses move and act faster based on the Hyper Speed Increase sliders."));
    AddWidget(path, "Hyper Speed Increase: +%d%%", WIDGET_CVAR_SLIDER_INT)
        .CVar(HyperSpeed::FullHealthCVar)
        .Options(IntSliderOptions()
                     .Min(0)
                     .Max(400)
                     .DefaultValue(HyperSpeed::FullHealthDefault)
                     .Format("+%d%%")
                     .Tooltip(
                         "Sets the Hyper speed increase while an enemy or boss is at full health.\n"
                         "10% runs one extra update every 10 frames, 100% is twice as fast, and 400% is five times "
                         "as fast."));
    AddWidget(path, "Hyper Speed at 0 Health: %+d%%", WIDGET_CVAR_SLIDER_INT)
        .CVar(HyperSpeed::ZeroHealthCVar)
        .Options(IntSliderOptions()
                     .Min(-100)
                     .Max(400)
                     .DefaultValue(HyperSpeed::ZeroHealthDefault)
                     .Format("%+d%%")
                     .Tooltip(
                         "Sets the Hyper speed increase reached as health approaches zero.\n"
                         "Values below the full-health setting taper actors back toward normal speed; Hyper mode "
                         "never makes them slower than normal."));
