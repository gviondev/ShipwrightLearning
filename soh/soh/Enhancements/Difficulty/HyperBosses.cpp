...
#include "overlays/actors/ovl_Boss_Sst/z_boss_sst.h"
    ...
    if (IsHyperBossesActive() && isBossActor && !Player_InBlockingCsMode(gPlayState, player)) {
        ...
        if (actor->id == ACTOR_BOSS_VA) {
            ...
            if (actor->id == ACTOR_EN_FHG_FIRE) {
                ...
                additionalUpdates = 0;
            } else if (actor->id == ACTOR_BOSS_SST && BossSst_ShouldUseNormalUpdateRate(actor)) {
                // Preserve readable vulnerability/rhythm windows and real-time hand-coordination barriers.
                additionalUpdates = 0;
            ...
            for (int32_t i = 0; i < additionalUpdates; ++i) {
                ...
                if (actor->id == ACTOR_BOSS_SST && BossSst_ShouldUseNormalUpdateRate(actor)) {
                    // Stop if a Hyper substep just crossed into protected Bongo timing or synchronization.
                    break;
                }
            }
        }
